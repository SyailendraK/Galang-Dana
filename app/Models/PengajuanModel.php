...
class PengajuanModel extends Model
{
  ...
  protected $allowedFields = ['user_id','cerita','siapa','nama','nik','nomorHP','email','alamat','bank','rekening','jumlah','fotoDiri','status'];
    ...
    public function getPengajuanTerbayarAll($limit)
    {
    ...
    public function cariBantuanByUser()
    {
        return $this->where(['user_id' => user_id(), 'status !=' => 2])->orderBy('created_at', 'desc')->findAll();
    }

    public function cariBantuanByUserTerbayar()
    {
        return $this->where(['user_id' => user_id(), 'status' => 2])->orderBy('created_at', 'desc')->findAll();
    }

    public function getBantuanByNIK($nik)
    {
        return $this->where(['nik' => $nik, 'status' => 1])->orderBy('created_at', 'desc')->findAll();
    }

    public function getBantuanByIDUser($id)
    {
        return $this->where(['user_id' => $id, 'status !=' => 2])->orderBy('created_at', 'desc')->findAll();
    }
    ...
    // {
    //     try {
    //         $this->where("id=".$id." AND (status=1 OR status=0)")->delete();
    //         return true;
    //     } catch (\Throwable $th) {
    //         return false;
    //     }
    ...
    public function updatePengajuanStatus($id,$status)
    {
        try {
